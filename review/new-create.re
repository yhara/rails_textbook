...
また、@<tt>{<%= render 'form', book: @book %>}の@<tt>{book: @book}の部分は、@<tt>{@book}変数を埋め込み先のパーシャル内で@<tt>{book}変数として使うための指示です（パーシャル内でも@はじまりのインスタンス変数を利用することも可能です。それでもわざわざbook変数として渡しているのは、パーシャル内で利用する変数を明示すること、他のコントローラでパーシャルを流用する時にインスタンス変数名を揃える必要がないことなどのメリットがあります。ちなみに、Rails 4.2では変数を渡さず、インスタンス変数をパーシャル内部で使うコードが生成されます）。
