...
newアクションは @<tt>{@books = Book.new} の1行です。Book.newでBookクラスのインスタンス（データは空っぽ）を作り、@bookインスタンス変数へ代入し、ビューへ渡します。Book.newでつくったBookクラスのインスタンスはタイトルとメモを格納できるようになっています。
...
renderメソッドは別のビューファイルを埋め込みます。埋め込む用のビューファイルをパーシャルと言います。埋め込むファイル名には1つルールがあり、renderで書いた文字列の先頭に_を付けたファイル名にします。つまり、@<tt>{<%= render 'form', book: @book %>}で埋め込まれるファイルは @<tt>{_form.html.erb} になります（わざわざ別のファイルに書いてある理由は、他の画面でも同じ部品を共用したいからです）。



また、@<tt>{<%= render 'form', book: @book %>}の@<tt>{book: @book}の部分は、@<tt>{@book}変数を埋め込み先のパーシャル内で@<tt>{book}変数として使うための指示です（パーシャル内でも@はじまりのインスタンス変数を利用することも可能です。それでもわざわざbook変数として渡しているのは、パーシャル内で利用する変数を明示すること、他のコントローラでパーシャルを流用する時にインスタンス変数名を揃える必要がないことなどのメリットがあります。）@<fn>{1}。
...
では、@<tt>{_form.html.erb} の中を解説していきます。この中で、前半2行目からのこの部分はエラーを表示するコードです。ここでは説明を省略して、それ以外の基本となる部分を説明します。
...
@<tt>{f.label :title} で "Title"という文字列を表示しています。その名の通り、ラベルの部分です。@<tt>{f.text_field :title} はその下にあるテキスト入力欄です。fはformブロック内の変数で、ここではbookに関するformを記述するために使っています（見慣れない書き方ですが、ここはそう書くものだと思ってもらえれば大丈夫です）。
...
メモの部分も同様です。@<tt>{f.label :memo} が "Memo" を表示する部分です。@<tt>{f.text_area :memo}がその下のテキスト入力欄を作ります。@<tt>{text_area} は先ほどの @<tt>{text_field} よりも広くて改行を入力できるテキスト入力欄を作るメソッドです。
...
@<tt>{f.submit} は投稿ボタン（Create Bookボタン）を作ります。このボタンを押すとform内の情報をまとめてサーバへ送信（リクエストを送信）します。つまり…
...
このパラメータを制限する仕組みはStrong Parametersと呼ばれます。これが必要な理由は、攻撃に対する防御、つまりセキュリティのためです。ブラウザから飛ばすパラメータは、ユーザーの手によって改ざんすることも可能です。つまり、任意のパラメータを飛ばす攻撃をすることができます。この1つ前のnewの画面で用意したフォームに存在しないパラメータが飛んでくる可能性があるので、ここで変更を許可するパラメータを絞っています（もう少し説明すると、ここで使われている@<tt>{Book.new(book_params)}に関係しています。newメソッドは、引数で受け取った値を自分のカラムへ代入します。たとえば、titleだけを更新したいケースがあったとして、book_paramsにtitleの情報だけがやってくればいいのですが、攻撃者はmemoの情報もパラメータとして飛ばすこともあります。StrongParameterで受け取り可能なパラメータを絞っていないと、プログラマの意図しないカラムが更新されてしまうことになります）。
...
//footnote[1][Rails 4.2では変数を渡さず、インスタンス変数をパーシャル内部で使うコードが生成されます。]
