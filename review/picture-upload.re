...
続いて、carrierwaveを利用可能にするために、carrierwaveが提供する以下のコマンドを実行して必要なファイルを作成します。@<tt>{bin/spring stop}コマンドはspringというキャッシュの仕組みを再起動します（このコマンドは環境によって実行不要な場合がほとんどですが、確実に成功するように実行しています）。@<tt>{rails g uploader Picture}コマンドはcarrierwaveを利用するのに必要なファイルを生成します。
