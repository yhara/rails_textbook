...
また、本書の目的のもう1つは、世の中にあるたくさんの情報を読むために前提となる基礎知識を身につけることです。Railsを学ぶための良い資料として、@<href>{http://railsguides.jp/,RailsGuides} や @<href>{http://railstutorial.jp/,RailsTutorial} があります。本書を読み進めると、これらの資料を読み進めるための基礎知識が身についてきます。実践的なRailsアプリを作るために必要なさまざまな資料を読むための入り口として本書はあります。
 ...
 * @<href>{http://dotinstall.com/lessons/basic_html_v3,ドットインストール　HTML入門}
 * @<href>{http://dotinstall.com/lessons/basic_css_v3,ドットインストール　CSS入門}
...
本書はRailsの入門的な本やサイトを読んで難しいと感じた方に、そこまでの知識を埋められるように書いています。逆に言えば、他の入門書を読める力があれば、本書は易しすぎると感じるかもしれません。具体的には、@<href>{http://railstutorial.jp/,RailsTutorial} を読み進められるようであれば、本書から得られるものは少ないかもしれません。
...
本書はRuby 2.4.1、Rails 5.1.2で動作確認をしています。@<fn>{1}
...
また、ブラウザはお好きなものをご利用いただけますが、途中のデバッグの箇所で @<href>{https://www.google.co.jp/chrome/browser/,Chrome} の機能を例にして説明していますので、Chromeを使うと分かり易いです。


=== ブラウザだけで開発できるクラウド開発環境 - Cloud9


@<href>{https://c9.io,Cloud9} はブラウザだけでRailsアプリ開発を行うことができます。セットアップはCloud9へ会員登録を行うだけです。有料プランもありますが、本書の内容で開発を行う範囲では無料プランの範囲で利用することができます。マシンを変えても同じ環境で開発でき、ブラウザが使える環境であればどの端末からでも開発できるというメリットがあります。一方でネットに接続していないと開発できないことや、Cloud9がサービスをやめてしまうと継続して使えないというデメリットもあります。
...
@<tt>{rails -v}コマンドでインストールしたrailsのバージョンが表示されればインストール成功です。結果に表示されるバージョンの数字は、実行時の最新バージョンになります。
...
Railsアプリはどこのフォルダへ置いても動作しますが、workspaceフォルダ以下に置くと画面左側のWorkspace欄にフォルダツリーが表示されて便利です。本書では@<tt>{my_web_apps}フォルダへRailsアプリを作成していきますので、これを@<tt>{workspace}に読み替えるのがお勧めです。
...
ここからはCloud9で最新版のRubyを使う方法を説明します。Cloud9はRubyの管理にrvmを利用しているので、rvmでの通常の手順でバージョンアップ可能です。@<fn>{2}
...
Rubyを新規インストールした場合は、そのあとで@<tt>{gem i rails}コマンドを実行してrailsをインストールします。
...
プログラムを入力する道具であるエディタは、好みのものがあればそれを利用してください。特にない場合は @<href>{https://atom.io/,Atom} をお勧めします。Atomは無料で利用することができます（ライセンスはMITライセンスです）。


=== ローカルに開発環境を作る - Windowsの場合


Windowsの場合は @<href>{http://railsinstaller.org,RailsInstaller} を利用することで環境を構築することができます。
...
もしも想定通り動作しない場合は、上記のCloud9を利用することも検討してみてください@<fn>{3}。



プログラムを入力する道具であるエディタは、好みのものがあればそれを利用してください。特にない場合は @<href>{https://github.com/atom/atom/blob/master/README.md#windows,Atom} をお勧めします。Atomは無料で利用することができます（ライセンスはMITライセンスです）。


==== RailsInstallerバージョン3.2.1を使う手順


バージョン3.2.1を使う手順を説明します。インストーラ（@<href>{https://s3.amazonaws.com/railsinstaller/Windows/railsinstaller-3.2.1.exe,rails_installer3.2.1.exe}）をダウンロードし、指示に従い実行していきます。オプションなどは初期状態のままで問題ありません。
...
バージョン3.3.0を使う手順を説明します。バージョン3.3.0を使う場合は、インストール後に簡単な修正作業を行う必要があります。インストーラ（@<href>{https://s3.amazonaws.com/railsinstaller/Windows/railsinstaller-3.3.0.exe,rails_installer3.3.0.exe}）をダウンロードし、指示に従い実行していきます。オプションなどは初期状態のままで問題ありません。
...
//footnote[1][2017年7月1日現在、Rubyの最新バージョンは2.4.1、Railsの最新バージョンは5.1.2です。]

//footnote[2][これでうまくいかない場合はrvm公式サイト@<href>{https://rvm.io/rvm/upgrading,https://rvm.io/rvm/upgrading}に掲載されている方法（@<tt>{\curl -sSL https://get.rvm.io | bash -s stable; rvm reload}）を試してからもう一度実行してみてください。]

//footnote[3][Railsのバージョンはコマンドプロンプトから @<tt>{gem update rails} コマンドで最新版にバージョンアップすることが可能ですが、Rubyのバージョンを上げるのは難しいためパッケージに入っているRubyをそのまま使うのが良いでしょう。最新バージョンのRubyおよびRailsを利用したい場合は、上記のCloud9の利用をお勧めします。]
