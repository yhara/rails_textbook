...
本書はRuby 2.3.1と、Rails 5.0.0およびRails4.2.4で動作確認をしています。@<fn>{1}
...
@<tt>{rails -v}コマンドでインストールしたrailsのバージョンを確認します。
...
以下はRubyのバージョンアップ方法です。少し難しい内容になりますので、はじめての方はここまでの内容で完了し、初期状態で利用する方が簡単です。cloud9ではRubyの管理にrvmを利用しているので、rvmでの通常の作法でバージョンアップ可能です。@<fn>{2}
...
インストール後、Railsアプリのルートフォルダへ .ruby-versionファイルを作成し、@<tt>{2.3.1} といった具合にRubyのバージョンを書くと、そのアプリではこのファイルで指定したバージョンを使用することができます。
...
Windowsの場合は @<href>{http://railsinstaller.org,RailsInstaller} を利用することで環境を構築することができます。インストーラをダウンロードして実行してください。



ターミナルの起動はWindowsのアプリケーション選択から "Command Prompt with Ruby and Rails" を選んでください。通常のコマンドプロンプトを起動すると、環境変数の設定があわずにrailsコマンドやrubyコマンドを使えない場合があります。もしも想定通り動作しない場合、たとえばrailsコマンドを実行したときに「内部コマンドまたは外部コマンド、操作可能なプログラムまたはバッチファイルとして認識されていません」や「指定されたパスが見つかりません」と言われた場合は、上記のCloud9を利用することも検討してみてください@<fn>{3}。
...
//footnote[1][2016年7月2日現在、Railsの最新バージョンは5.0.0です。本書の内容はRails 4.2でも同様に実行できますので、Rails 4.2をお使いいただいても大丈夫です。Rails4.2でRails5.0と異なる部分については、その都度補足していきます。]
...
//footnote[3][2016年2月現在の最新パッケージは、Ruby 2.2.4, Rails 4.2という構成になっています。Railsはコマンドプロンプトから @<tt>{gem update rails} コマンドで最新版にバージョンアップすることが可能です。しかし、Rubyのバージョンを上げるのは簡単ではないので、パッケージに入っているRubyをそのまま使うのが良いでしょう。最新バージョンのRubyおよびRailsを利用したい場合は、上記のCloud9の利用をお勧めします。]
